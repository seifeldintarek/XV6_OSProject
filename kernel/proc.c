...
// Add this function (You can place it near the original wait)
int
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          // --- FILL THE STRUCT ---
          st.pid = pp->pid;
          st.turnaround_time = pp->turnaround_time; // Using your variable names
          st.creation_time = ticks;
...
void update_time()
{
  // Implement any logic here to update parameters related to time(waiting time,varyig time,etc...)
  struct proc *p;
  for (p = proc; p < &proc[NPROC]; p++)
  {
    acquire(&p->lock);
...
int getptable(int nproc, uint64 buffer)
{
  ...
  for (p = proc; p < &proc[NPROC] && count < nproc; p++)
  {
    ...
    if (p->state != UNUSED)
    {
      // Fill the process info structure
      pinfo.pid = p->pid;
      ...
      // Copy to user space
      if (copyout(myproc()->pagetable, buffer + count * sizeof(struct proc_info),
